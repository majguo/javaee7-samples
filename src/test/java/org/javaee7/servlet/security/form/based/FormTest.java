...
public class FormTest {
    ...
    public void testGetWithIncorrectCredentials() throws Exception {
        ...
        // Get the page after submit and handle multiple types
        Object resultPage = submitButton.click();
        
        if (resultPage instanceof HtmlPage) {
            System.out.println("HTML Page: " + ((HtmlPage)resultPage).getTitleText());
            assertEquals("Form-Based Login Error Page", ((HtmlPage)resultPage).getTitleText());
        } else {
            System.out.println("Text Page: " + ((TextPage)resultPage).getContent());
            assertEquals("my POST for ErrorServlet", ((TextPage)resultPage).getContent());
        }
    }
}
